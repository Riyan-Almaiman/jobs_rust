...
impl SqliteStorage {
    ...
    async fn migrate(&self) -> std::result::Result<(), sqlx::Error> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_jobs_queue_state ON jobs(queue, state)")
            .execute(&self.pool)
            .await?;
    ...
    fn row_to_job(&self, row: sqlx::sqlite::SqliteRow) -> Result<Job> {
        ...
        Ok(Job {
            ...
            job_type,
            payload,
            state: JobState::from_db(&state_str, error, run_at),
    ...
    fn row_to_recurring(&self, row: sqlx::sqlite::SqliteRow) -> Result<RecurringJob> {
        ...
        Ok(RecurringJob { id, cron, job_type, payload, queue, next_run })
    }
}
...
impl Storage for SqliteStorage {
    ...
    async fn update_state(&self, id: &JobId, state: JobState) -> Result<()> {
        ...
        sqlx::query("UPDATE jobs SET state = ?, error = ?, run_at = ?, updated_at = ? WHERE id = ?")
            .bind(state_str)
            .bind(error)
            .bind(run_at)
            .bind(Utc::now().to_rfc3339())
    ...
    async fn increment_retries(&self, id: &JobId, state: JobState) -> Result<()> {
        ...
        sqlx::query(
            "UPDATE jobs SET state = ?, error = ?, run_at = ?, retries = retries + 1, updated_at = ? WHERE id = ?",
        )
        .bind(state_str)
        .bind(error)
        .bind(run_at)
    ...
    async fn get_jobs_by_state(&self, state: &str, limit: u32) -> Result<Vec<Job>> {
        let rows = sqlx::query("SELECT * FROM jobs WHERE state = ? ORDER BY updated_at DESC LIMIT ?")
            .bind(state)
            .bind(limit as i32)
            .fetch_all(&self.pool)
            .await?;
    ...
    async fn enqueue_scheduled_jobs(&self, before: DateTime<Utc>) -> Result<()> {
        sqlx::query(
            ...
            WHERE state = 'scheduled' AND run_at <= ? AND parent_id IS NULL
            "#,
        )
        .bind(Utc::now().to_rfc3339())
    ...
    async fn fetch_due_recurring(&self, now: DateTime<Utc>) -> Result<Vec<RecurringJob>> {
        let rows = sqlx::query("SELECT * FROM recurring_jobs WHERE next_run <= ?")
            .bind(now.to_rfc3339())
            .fetch_all(&self.pool)
            .await?;

        rows.into_iter().map(|row| self.row_to_recurring(row)).collect()
    }

    async fn promote_recurring_job(
        &self,
        id: &str,
        ...
        if let Some(row) = row {
            ...
            sqlx::query(
                r#"
                INSERT INTO jobs (id, queue, job_type, payload, state, retries, created_at, updated_at, max_retries)
                VALUES (?, ?, ?, ?, 'enqueued', 0, ?, ?, 3)
                "#,
            )
            .bind(&new_id.0)
